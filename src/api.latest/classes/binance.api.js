...
class BinanceAPI extends EventEmitter {
  ...
  getKlines(symbol = this.state.symbol, interval = this.state.interval, limit = this.state.limit) {
    this.api('GET', '/klines', { query: { symbol, interval, limit } })
      ...
      .then((klines) => this.updateKlines(klines))
  }

  updateKlines(klines = []) {
    this.state.klines = klines
    return Promise.resolve(Array.from(klines))
  ...
  orderTest(side = '', symbol = this.state.symbol, type = 'MARKET', quantity = 1, timestamp = Date.now()) {
    this.emit('orderTest', this.createParams({ symbol, side, type, quantity }, timestamp))
  }


  tickerPrice(symbol = this.state.symbol) {
    return this.api('GET', '/ticker/price', { symbol })
  }

  buy(symbol = this.state.symbol, type = 'MARKET', quantity = 1, timestamp = Date.now()) {
    this.tickerPrice(symbol).then((data) => console.log('tickerPrice', { data }))
